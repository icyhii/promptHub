/*
  ...
  1. New Tables
    - `prompt_versions` table for storing version history
      - `id` (uuid, primary key)
  ...
  2. Indexes
    - Index on prompt_id for faster lookups
    - Composite index on prompt_id and version_number
    
  3. Security
    - Enable RLS
    - Add policies for version creation and viewing
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE indexname = 'idx_prompt_versions_prompt_id'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE indexname = 'idx_prompt_versions_prompt_id_version'
  ) THEN
